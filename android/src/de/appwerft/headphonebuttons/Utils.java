...
public final class Utils {
	...
	// private static final String GOOGLE_MUSIC_RECEIVER =
	// "com.google.blahdfdf";
	 ...
	 */
	public static void forwardKeyCodeToComponent(Context context,
			ComponentName selectedReceiver, boolean launch, int keyCode,
			BroadcastReceiver cleanUpReceiver) {

		Intent mediaButtonDownIntent = new Intent(Intent.ACTION_MEDIA_BUTTON);
		KeyEvent downKe = new KeyEvent(SystemClock.uptimeMillis(),
		...
		KeyEvent upKe = new KeyEvent(SystemClock.uptimeMillis(),
		 ...
		 * "Forwarding Down and Up intent events to " + selectedReceiver +
		 * " Down Intent: " + mediaButtonDownIntent + " Down key:" + downKe +
		 * " Up Intent: " + mediaButtonUpIntent + " Up key:" + upKe);
		 */
		...
		context.sendOrderedBroadcast(mediaButtonDownIntent, null,
				cleanUpReceiver, null, Activity.RESULT_OK, null, null);
		context.sendOrderedBroadcast(mediaButtonUpIntent, null,
				cleanUpReceiver, null, Activity.RESULT_OK, null, null);
	...
	public static List<ResolveInfo> getMediaReceivers(
		...
		List<ResolveInfo> mediaReceivers = packageManager
				.queryBroadcastReceivers(mediaButtonIntent,
						PackageManager.GET_INTENT_FILTERS
								| PackageManager.GET_RESOLVED_FILTER);
		if (filterHidden) {

			String hiddenReceiverIdsString = PreferenceManager
					.getDefaultSharedPreferences(context).getString(
							Constants.HIDDEN_APPS_KEY, "");
			List<String> hiddenIds = Arrays.asList(hiddenReceiverIdsString
					.split(","));

			for (int i = mediaReceivers.size() - 1; i >= 0; i--) {
				ResolveInfo mediaReceiverResolveInfo = mediaReceivers.get(i);

				if (hiddenIds.contains(getMediaReceiverUniqueID(
						mediaReceiverResolveInfo, packageManager))) {
	...
	public static String getMediaReceiverUniqueID(ResolveInfo resolveInfo,
			PackageManager packageManager) {
		String receiverId = resolveInfo.activityInfo.name;
		// Don't think the following is an issue anymore with the latest
		// versions of google play, if it is i'll add back
		...
		// receiverId = resolveInfo.activityInfo.applicationInfo.sourceDir +
		// receiverId;
		// }
		return receiverId;
	}
	...
	public static String getAppName(ResolveInfo resolveInfo,
			PackageManager packageManager) {
		return resolveInfo.activityInfo.applicationInfo.loadLabel(
